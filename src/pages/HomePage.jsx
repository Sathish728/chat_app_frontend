...
import NoFriendsFound from "../components/NoFirendsFound";
...
const HomePage = () => {
  ...
  // ✅ Set outgoing request IDs for disabling buttons
  useEffect(() => {
  ...
  try {
    const resultAction = await dispatch(sendFriendRequest(userId));

    if (sendFriendRequest.fulfilled.match(resultAction)) {
      // ✅ Immediately update local state so button changes instantly
                    ...
                    <div className="card-body p-5 space-y-4">
                      ...
                      >
                        {hasRequestBeenSent ? (
                          ...
                          <>
                            <UserPlusIcon className="size-4 mr-2 animate-spin" />
                            Sending...
                          </>
                        ) : (
                          <>
